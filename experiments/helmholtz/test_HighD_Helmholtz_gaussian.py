...
#
# $$ u = \exp\!\left(-\sum_{i=1}^{d} x_i^2\right) $$
#
# Domain: [-2, 2]^d  (purely spatial, no time)
# Boundary conditions: u = exp(-4*(d-1)*x_j^2 - 4) at each face — non-zero.
#
# Key derivatives:
#   u_{x_i x_i} = (-2 + 4*x_i^2) * u
#   Delta u = u * sum_i(-2 + 4*x_i^2)
#   f = u^2 - Delta u = u^2 - u*sum_i(-2 + 4*x_i^2)


def data_gen(x):
    # exact solution to Helmholtz equation
    sol = torch.exp(-torch.sum(x ** 2, dim=1).view(-1, 1))
    return sol
...
def forcing(x):
    ...
    u = data_gen(x)
    lap = u * torch.sum(-2 + 4 * x ** 2, dim=1).view(-1, 1)
    return u ** 2 - lap


def bound_data(n, d):
    # sample points on the boundary of [-2, 2]^d
    n0 = math.floor(n / d / 2)
    x = torch.empty(n, d)
    for i in range(d):
        x0 = 4 * torch.rand(n0, d) - 2.
        x0[:, i] = -2.
...
def _sample_interior(N, d, device):
    return (4 * torch.rand(N, d) - 2).to(device)
...
plt.ylabel("Loss")
plt.title(f"{d}d Helmholtz Equation (u²−Δu=f) – Gaussian solution")
fig.savefig("HighD_Helmholtz_gaussian.jpg", dpi=500)
