...
#
# $$ u = \prod_{i=1}^{d} \cos(\pi x_i) $$
#
# Domain: [-1/2, 1/2]^d  (purely spatial, no time)
# Boundary conditions: u = 0 on all faces since cos(±pi/2) = 0.
#
# Key derivatives:
#   u_{x_i x_i} = -pi^2 * u   =>  Delta u = -d*pi^2 * u
#   f = u^3 - Delta u = u^3 + d*pi^2 * u


def data_gen(x):
    # exact solution to Helmholtz equation
    sol = torch.prod(torch.cos(math.pi * x), dim=1).view(-1, 1)
    return sol
...
def bound_data(n, d):
    # sample points on the boundary of [-1/2, 1/2]^d
    n0 = math.floor(n / d / 2)
    x = torch.empty(n, d)
    for i in range(d):
        x0 = torch.rand(n0, d) - 0.5
        x0[:, i] = -0.5
...
for repeat in range(num_repeats):
    ...
    x_to_train_f = (torch.rand(N_f, d) - 0.5).to(device)
    x_val = (torch.rand(500, d) - 0.5).to(device)
...
plt.ylabel("Loss")
plt.title(f"{d}d Helmholtz Equation (u³−Δu=f) – Cosine product solution")
fig.savefig("HighD_Helmholtz_cosine_product.jpg", dpi=500)
