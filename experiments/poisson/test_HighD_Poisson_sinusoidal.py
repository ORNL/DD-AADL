...
# Boundary conditions: u = 0 on all faces (sin vanishes at 0 and 1).
#
# Key derivatives:
#   u_{x_i x_i} = -pi^2 * u   =>  Delta u = -d*pi^2 * u
#
# Forcing:
#   f = -Delta u = d*pi^2 * u
...
def data_gen(x):
    # exact solution to Poisson equation
    sol = torch.prod(torch.sin(math.pi * x), dim=1).view(-1, 1)
    return sol
...
for repeat in range(num_repeats):
    ...
    record[0, repeat] = loss_poisson(x, y, x_to_train_f, d, net)[1].detach()

    for itr in range(1, niters + 1):
        optim.zero_grad()
        loss = loss_poisson(x, y, x_to_train_f, d, net)[1]
        loss.backward()
        optim.step()
    ...
    for itr in range(1, niters + 1):
        def closure():
            with torch.enable_grad():
                optim.zero_grad()
                _, loss = loss_poisson(x, y, x_to_train_f, d, net)
                loss.backward()
                _last_loss[0] = loss
                return loss
        optim.step(closure)
...
for repeat in range(num_repeats):
    ...
    for itr in range(1, niters + 1):
        def closure():
            optim.zero_grad()
            res, loss = loss_poisson(x, y, x_to_train_f, d, net)
            loss.backward()
            _last_loss[0] = loss
            return res, loss
        optim.step(closure)
...
plt.ylabel("Loss")
plt.title(f"{d}d Poisson Equation (−Δu = f) – Sinusoidal solution")
fig.savefig("HighD_Poisson_sinusoidal.jpg", dpi=500)
