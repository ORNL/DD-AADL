...
# Boundary conditions: u = 0 on all spatial faces (sine vanishes at 0 and 1).


def data_gen(x):
    # exact solution to Burgers equation
    d = x.shape[1]
    xx = x[:, :d - 1]   # spatial dims
    sol = torch.exp(-x[:, -1].view(-1, 1)) * torch.prod(torch.sin(math.pi * xx), dim=1).view(-1, 1)
    return sol


def forcing(x):
    # forcing term: f = u_t + u * sum_i u_{x_i} - Delta_x u
    #
    # u_t = -u
    # u_{x_i} = exp(-t) * pi * cos(pi*x_i) * prod_{j!=i} sin(pi*x_j)
    # u_{x_i x_i} = -pi^2 * u  =>  Delta_x u = -(d-1)*pi^2 * u
    # conv = sum_i u * u_{x_i}
    d = x.shape[1]
    n = x.shape[0]
    xx = x[:, :d - 1]                              # (n, d-1)
    exp_t = torch.exp(-x[:, -1].view(-1, 1))       # (n, 1)
    sin_xx = torch.sin(math.pi * xx)               # (n, d-1)
    cos_xx = torch.cos(math.pi * xx)               # (n, d-1)

    u = exp_t * torch.prod(sin_xx, dim=1).view(-1, 1)
    ut = -u

    # convective term: sum_i u * u_{x_i}
    conv = torch.zeros(n, 1, device=x.device)
    for i in range(d - 1):
        mask = [j for j in range(d - 1) if j != i]
        prod_others = (torch.prod(sin_xx[:, mask], dim=1).view(-1, 1)
                       if mask else torch.ones(n, 1, device=x.device))
        u_xi = exp_t * math.pi * cos_xx[:, [i]] * prod_others
        conv = conv + u * u_xi

    # Laplacian: -(d-1)*pi^2 * u
    lap = -(d - 1) * math.pi ** 2 * u
...
def bound_data(n, d):
    # sample points on the boundary of [0,1]^{d-1} x [0,1]
    # spatial faces at x_i=0,1 plus initial condition at t=0
    n0 = math.floor(n / d / 2)
    x = torch.empty(n, d)
    for i in range(d - 1):
...
plt.ylabel("Loss")
plt.title(f"{d}d Burgers Equation – Sinusoidal solution")
fig.savefig("HighD_Burgers_sinusoidal.jpg", dpi=500)
